...
class Sensor(ABC):
    ...
    def isMeasWithinSoftThresh(self):
        if self.sensor_measurement > self.sensor_warn_lo_soft_thres and self.sensor_measurement < self.sensor_warn_hi_soft_thres :
            return True
        else:
            return False
    def isMeasWithinHardThresh(self):
        if self.sensor_measurement > self.sensor_warn_lo_hard_thresh and self.sensor_measurement < self.sensor_warn_hi_hard_thresh :
            return True
        else:
            return False
    @abstractmethod
